...
SELECT COUNT(patient_id) as total_number
FROM admissions;
...
SELECT *
FROM admissions
WHERE CAST(admission_date AS DATE) = CAST(discharge_date AS DATE);
...
-- SELECT   WHERE   DISTINCT    GROUP BY    HAVING
SELECT *
FROM patients
GROUP BY city
...
-- 2.Show unique first names from the patients table which only occurs once in the list. For example, 
   --if two or more people are named 'John' in the first_name column then don't include their name in 
...
--1 Show all of the patients grouped into weight groups. Show the total amount of patients in
...
--3 Show patient_id, first_name, last_name, and attending doctor's specialty. Show only the patients who 
--  has a diagnosis as 'Epilepsy' and the doctor's first name is 'Lisa' Check patients, admissions, and 
--  doctors tables for required information.
